    ...
    with accelerator.main_process_first():
        if config.get("pretrained_model", "") != "":
            model, optimizer, start_epoch, iters = load_checkpoint(
                model, optimizer, config["pretrained_model"], load_only_params=config.get("load_only_params", True)
            )
        else:
            start_epoch = 0
            iters = 0
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            with torch.no_grad():
                ...
                attn_mask = (
                    attn_mask.float()
                    * (~text_mask).unsqueeze(-1).expand(text_mask.shape[0], text_mask.shape[1], mask.shape[-1]).float()
                )
