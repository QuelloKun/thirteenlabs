...
class SLMAdversarialLoss(torch.nn.Module):
    ...
    def forward(
        self, iters, y_rec_gt, y_rec_gt_pred, waves, mel_input_length, ref_text, ref_lengths, use_ind, s_trg, ref_s=None
    ):
        ...
        for _s2s_pred, _text_length in zip(d, ref_lengths):
            ...
            out = torch.nn.functional.conv1d(
                _s2s_pred_org.unsqueeze(0), h.unsqueeze(1), padding=h.shape[-1] - 1, groups=int(_text_length)
            )[..., :l]
