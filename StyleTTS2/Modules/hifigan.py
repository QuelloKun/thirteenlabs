...
class Generator(torch.nn.Module):
    def __init__(
        ...
        self.m_source = SourceModuleHnNSF(
            sampling_rate=24000, upsample_scale=np.prod(upsample_rates), harmonic_num=8, voiced_threshod=10
        )
            ...
            if F0_down:
                F0_curve = (
                    nn.functional.conv1d(
                        F0_curve.unsqueeze(1), torch.ones(1, 1, F0_down).to("cuda"), padding=F0_down // 2
                    ).squeeze(1)
                    / F0_down
                )
            if N_down:
