    ...
    def __init__(self, layer_type, dim_in):
        ...
        if self.layer_type == "none":
            ...
            self.conv = nn.ConvTranspose2d(
                dim_in, dim_in, kernel_size=(3, 1), stride=(2, 1), groups=dim_in, output_padding=(1, 0), padding=(1, 0)
            )
        elif self.layer_type == "half":
            self.conv = nn.ConvTranspose2d(
                dim_in, dim_in, kernel_size=(3, 3), stride=(2, 2), groups=dim_in, output_padding=1, padding=1
            )
        else:
            raise RuntimeError(
...
class DurationEncoder(nn.Module):
    ...
    def __init__(self, sty_dim, d_model, nlayers, dropout=0.1):
        ...
        for _ in range(nlayers):
            self.lstms.append(
                nn.LSTM(
                    d_model + sty_dim, d_model // 2, num_layers=1, batch_first=True, bidirectional=True, dropout=dropout
                )
            )
...
def load_ASR_models(ASR_MODEL_PATH, ASR_MODEL_CONFIG):
    ...
    def _load_model(model_config, model_path):
        ...
        # didn't work without weights_only=False
